...
line_style2 = dict(color='black',dash='dot')

fig.add_trace(
  go.Scatter3d(x=ones*5,y=zeros,z=P_X05, mode='lines',line=line_style2)
)

fig.add_trace(
  go.Scatter3d(x=zeros,y=ones*5,z=P_X05, mode='lines',line=line_style2)
)
...
fig.add_trace(
  go.Scatter3d(x=ones*3,y=ones*4,z=P_X05, mode='lines',line=line_style2)
)

fig.add_trace(
  go.Scatter3d(x=ones*-4,y=ones*-3,z=P_X05, mode='lines',line=line_style2)
)

fig.add_trace(
  go.Scatter3d(x=x,y=y,z=ones*P_X5, mode='lines',line=line_style2)
)

camera = dict(
    up=dict(x=0, y=0, z=1),
    center=dict(x=0, y=0, z=0),
    eye=dict(x=0.4, y=-0.5, z=0.9)
)

fig.update_layout(showlegend=False, scene_camera=camera)
...
Since $Y$ is i.i.d, then the probability density $g(Y=0)$ is constant regardless of $X$ (i.e. $P_Y(0) = \gamma$ where $\gamma$ is some constant). We can similarly denote the constant $P_X(0) = \lambda$.
...
:::{.column-margin}
The integral $\int_{-\infty}^{\infty}{e^{-z^2}}{dz} = \sqrt{\pi}$ is a special case of the gaussian integral. [This video is a great explanation](https://www.youtube.com/watch?v=l27xKSNad2Y) if you are interested in its derivation.
:::
And thus:
