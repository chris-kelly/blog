...
author: "Chris Kelly"
date: '11-07-24'
image: '../images/fundamentals_clt.png'
...
format:
  html:
    ...
    other-links:
      ...
      - text: Deriving the gaussian intergral
...
::: {.callout-tip}
So we are looking to show that the characteristic function of $\bar{X}_n$ converges to the characteristic function of the standard normal distribution as $n \rightarrow \infty$.
:::
...
Howwever, it does make it more tricky to prove that the sample mean converges to a normal distribution: since as we increase $n$, the variance shinks, so the variance is not constant. Instead, we can scale and center the sample mean by $\sqrt{n}$ to ensure its variance is constant for a given sample size $n$:
...
We can formulate our characteristic function of the random variable $X_i/\sqrt{n}$ as a taylor series (if you want a refresher on Taylor series, see the bottom of the page):

$$
\displaylines{
\begin{align}
\mathbb{E}\left[\exp{\left\{\frac{it}{\sqrt{n}}(X_i-\mu)\right\}}\right] 
...
So we can now plug that into the characteristic function for $Z_n$, and take it to the limit as we increase the sample size $n$:
...
And this is identical to the characteristic function of the normal distribution where $\mu = 0$.
...
As a refresher, the Taylor series is a way to approximate a function as an infinite sum of terms. The $n$th term of the Taylor series is given by:
$$
\displaylines{
\begin{align}
