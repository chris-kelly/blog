...
$\xi_i$ is a shape parameter, governing the shape of the distribution (e.g. in poisson, the expected value is the mean $\xi_i=\mu_i$). 

$\phi$ is a dispersion parameter, although it is not always necessary (e.g. gaussian has the standard deviation $\phi = \sigma$, but in poisson the variance is equated to the expected rate so not required).
...
---|---|---
Count | $y_i \sim Pois(\mu_i)$<br>$y_i \sim \frac{\mu_i^{y} \times e^{-\mu_i}}{y!}$ | $g(\mu) = \ln{[\mu_i]}$<br>(log-link)
Binary | $y_i \sim Bern(\mu_i)$<br>$y_i \sim \mu_i^y \times (1-\mu_i)^{(1-y)}$ | $g(\mu) = \ln{\left[\frac{\mu_i}{1-\mu_i}\right]}$<br>(logit-link)
Binary | $y_i \sim Bern(\mu_i)$<br>$y_i \sim \mu_i^y \times (1-\mu_i)^{(1-y)}$ | $g(\mu) = \Phi^{-1}[\mu_i]$<br>(probit-link)
Normal | $y_i \sim N(\mu_i,\sigma^2)$<br>$y_i \sim \frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{1}{2}\left(\frac{y_i-\mu_i}{\sigma^2}\right)^2}$ | $g(\mu) = \mu$<br>(identity-link)

Now all of these probability density functions can be reformulated as being of the exponential family form!
